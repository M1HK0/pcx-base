class CSliderInt {
    ...
    CSliderInt(const string &in _name, int _min, int _max, int _default, int _step = 1, const string &in _suffix = "") {
        name = _name;
        min_value = _min;
        max_value = _max;
        value = _default;
    ...
    void handle_input(float x, float y, float w) {
        ...
        if (cursor::m1_pressed) {
            if (cursor::is_hovered(vector2(minus_x, slider_y), vector2(minus_size.x, 12)))
                value = int(clamp(float(value - step), float(min_value), float(max_value)));
        ...
        if (dragging) {
            ...
            float raw = float(min_value) + pct * float(max_value - min_value);
            int steps = int((raw - float(min_value)) / float(step) + 0.5f);
            value = int(clamp(float(min_value + steps * step), float(min_value), float(max_value)));
        }
    }
    ...
    void render(float x, float y, float w) {
        ...
        visual_value = lerp(visual_value, float(value), 0.15f);
        float pct = (visual_value - float(min_value)) / float(max_value - min_value);
        ...
        string val_text = formatInt(value) + suffix;
