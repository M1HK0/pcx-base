...
nlohmann::json element_to_json(const std::shared_ptr<CElement>& el) {
    ...
    case pcx::EElementType::KEYBIND:
        entry["type"] = "keybind";
        entry["key"] = el->keybind->key;
        entry["mode"] = static_cast<int>(el->keybind->mode);
        entry["toggle"] = el->keybind->toggle_state ? 1.0 : 0.0;
        break;
    case pcx::EElementType::DROPDOWN:
        entry["type"] = "dropdown";
        entry["value"] = el->dropdown->selected;
        break;
    case pcx::EElementType::MULTISELECT: {
        entry["type"] = "multiselect";
        nlohmann::json sel;
        for (size_t j = 0; j < el->multiselect->selected.size(); ++j)
            sel[std::to_string(j)] = el->multiselect->selected[j] ? 1.0 : 0.0;
        entry["selected"] = sel;
        break;
    }
...
void apply_json(const std::shared_ptr<CElement>& el, const nlohmann::json& entry) {
    ...
    case pcx::EElementType::SLIDER_FLOAT:
        ...
        break;
    case pcx::EElementType::KEYBIND:
        el->keybind->key = entry.value("key", 0);
        el->keybind->mode = static_cast<pcx::EKeybindMode>(entry.value("mode", 0));
        el->keybind->toggle_state = entry.value("toggle", 0.0) > 0.5;
        break;
    case pcx::EElementType::DROPDOWN:
    ...
    case pcx::EElementType::MULTISELECT:
        if (entry.contains("selected")) {
            const auto& sel = entry["selected"];
            for (size_t j = 0; j < el->multiselect->selected.size(); ++j) {
                const std::string key = std::to_string(j);
                if (sel.contains(key))
                    el->multiselect->selected[j] = sel[key].get<double>() > 0.5;
            }
        }
        break;
    case pcx::EElementType::COLORPICKER: {
...
void refresh_listbox(const std::shared_ptr<CListBox>& list) {
    ...
    if (pcx::query_directory(base_path, {".cfg"}, files)) {
        for (const auto& f : files) {
            if (f.size() > 4)
                list->add_item(f.substr(0, f.size() - 4));
        }
    }
}
...
bool save(const std::string& name) {
    ...
    return pcx::create_file(base_path + "\\" + name + ".cfg", pcx::base64_encode(json));
}

bool load(const std::string& name) {
    std::string raw;
    if (!pcx::read_file(base_path + "\\" + name + ".cfg", raw)) return false;
    std::string json;
...
bool remove(const std::string& name) {
    return pcx::delete_file(base_path + "\\" + name + ".cfg");
}
